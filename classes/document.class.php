...
class document extends crownfire {
	...
	public function getBlankDocumentsByPropertyId($property_id, $state = null, $group = null, $year = null) {
        ...
        public function getBlankDocumentsByUserId($user_id) {
		$query = "SELECT documents.id FROM documents WHERE documents.user_id = '".$this->db->escape_string($user_id)."'  AND documents.is_blank_document=1";
		
		$query .= ' ORDER BY documents.date_added DESC';
                //echo $query;
		$results = $this->db->dbQuery($query);
		$return = array();
		while($myrow = $this->db->row($results)) {
